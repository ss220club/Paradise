...
/client/verb/link_discord_account()
	...
	if(prefs && prefs.discord_id && length(prefs.discord_id) < 32)
		to_chat(usr, "<span class='darkmblue'>Аккаунт Discord уже привязан! Чтобы отвязать используйте команду <span class='boldannounce'>!отвязать_аккаунт</span> в канале <b>#bot</b> в Discord-сообществе!</span>")
		return
	...
	if(SSdbcore.IsConnected())
		...
		if(!query_update_token.warn_execute())
			...
			return
		qdel(query_update_token)
		to_chat(usr, "<span class='darkmblue'>Для завершения используйте команду <span class='boldannounce'>!привязать_аккаунт [token]</span> в канале <b>#bot</b> в Discord-сообществе!</span>")
		if(prefs)
			prefs.load_preferences(usr)
