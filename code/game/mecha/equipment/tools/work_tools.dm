...
/obj/item/mecha_parts/mecha_equipment/cable_layer/get_equip_info()
	var/output = ..()
	if(output)
		return "[output] \[Cable: [cable ? cable.amount : 0] m\][(cable && cable.amount) ? "- <a href='?src=[UID()];toggle=1'>[!equip_ready?"Деа":"A"]ктивировать</a>|<a href='?src=[UID()];cut=1'>Разрезать</a>" : null]"
	return
